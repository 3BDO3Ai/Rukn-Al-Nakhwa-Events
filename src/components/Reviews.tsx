              ...
              >
                {localizedReviews.map((review, index) => (
                  <article
                    ...
                    className={`w-[330px] sm:w-[360px] lg:w-[390px] min-h-[260px] shrink-0 rounded-3xl border border-gold/25 bg-gradient-to-br from-white/[0.1] via-white/[0.06] to-transparent backdrop-blur-xl p-7 shadow-[0_18px_36px_rgba(0,0,0,0.28)] flex flex-col justify-between ${isArabic ? "text-right" : "text-left"}`}
                    lang={isArabic ? "ar" : "en"}
                    dir={isArabic ? "rtl" : "ltr"}
                  >
                    <div>
                      <p className="text-gold text-5xl leading-none mb-4">“</p>
                      <p className={`text-white/90 leading-relaxed ${isArabic ? "text-[1.02rem] font-semibold" : "text-base"}`}>
                        {review.text}
                      </p>
                    </div>

                    <div className="pt-6 mt-6 border-t border-gold/20">
                      <span className="inline-flex items-center rounded-full border border-gold/35 bg-gold/12 px-4 py-1.5 text-gold font-extrabold tracking-wide">
                        {review.name}
                      </span>
                    </div>
                  </article>
